...
echo $from ."   " .$stype."  " .$page;
if ($user_name=='admin'){
  if($from == "corpus_correction"){
  header("Location: ../corpus_correction.php");  
  }
  elseif($from == "search"){
    header("Location: http://localhost/corpus_api/test.php?key=$key&searchtype=$stype&page=$page");
  }
  elseif($from == "admin_corpus"){ 
